	...
	function trx_addons_ajax_sc_pagination() {
		...
		if ( ! empty( $_POST['params'] ) && is_string( $_POST['params'] ) ) {
			$params = substr( $_POST['params'], 0, 2 ) == 'a:'
						? trx_addons_unserialize( wp_unslash( $_POST['params'] ) )	// Old format (params are serialized - if an old template 'templates/tpl.sc_item_pagination.php' copied to the theme/skin)
						: json_decode( wp_unslash( $_POST['params'] ), true );		// New format (params are encoded to JSON)
		}
		if ( ! is_array( $params ) ) {
			$params = array();
		}
