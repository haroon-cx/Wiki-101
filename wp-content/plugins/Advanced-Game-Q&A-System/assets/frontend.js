jQuery(document).ready(function ($) {
  ...
  $('form[data-inited-validation="1"]').each(function () {
    ...
    $form.on("click", "input#confirm-submit-popup-button", function (e) {
      let isValid = true;
      $form.find(".error-message").remove();
      $form.find(".error-field").removeClass("error-field");
      $form.find(".submitted-successfully").remove(); // remove old success messages
...
addBtn.on("click", function () {
  editMode = false;
  editTarget = null;
  firstNameInput.val("").attr("placeholder", "Description"); // Reset placeholder to "Description"
...
$("#save-custom-field").on("click", function (e) {
  ...
  // Condition 1: Check if any characters have been entered
  if (!value) {
    alert("Please enter a valid contact number. Letters or unsupported symbols are not allowed.");
    return;
  }

  // Condition 2: Check if the input length is less than or equal to 50 characters
  if (value.length > 50) {
    alert("Maximum length for this field is 50 characters. Please shorten your input.");
    return;
  }
...
checkFieldLimit();
  ...
  $("textarea#notes-detail").each(function () {
    const $textarea = $(this);
  ...
  const maxCharsS = 3000;
  const typingDelayS = 2000; // 2s after typing stops

  // Iterate over each textarea (in case there are multiple)
  $("textarea.editor-faq").each(function () {
    const $textarea = $(this);
    const $charCounter = $textarea.siblings(".char-counter");
    const $currentCount = $charCounter.find(".current-count");
    const $formResponse = $textarea.siblings(".form-response");

    let typingTimer = null; // make timer local per textarea

    // On input, run the logic
    $textarea.on("input", function () {
      clearTimeout(typingTimer); // Clear the previous timeout to reset the delay

      let text = $textarea.val();

      // Cap text at maxCharsS
      if (text.length > maxCharsS) {
        $textarea.val(text.substring(0, maxCharsS)); // Trim input to maxCharsS if needed
        text = $textarea.val(); // Re-assign the text to the truncated version
      }

      const length = text.length;
      $currentCount.text(length); // Update current character count

      // Hide any previous success message immediately when typing
    ...
    // Clear the response and counter when the form is submitted
    $textarea.closest("form").on("submit", function (e) {
      e.preventDefault(); // Prevent form from submitting for demo purposes
      $formResponse.text("").removeClass("error success"); // Clear message
  ...
  $(document).on("click", ".custom-select-dropdown-lists li", function () {
    ...
    $(".custom-select-dropdown").next(".error-message").remove();
  });
  ...
  $(".agqa-popup-form-button").on("click", function () {
    $(".custom-select-dropdown-lists").slideUp(300);
    $(".custom-select-dropdown-title").removeClass("active");
  });
